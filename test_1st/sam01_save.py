...
# kospi200의 거래량
for i in range(len(hite.index)):     # 거래량 str -> int 변경
        hite.iloc[i,4] = int(hite.iloc[i,4].replace(',', ''))  
# 삼성전자의 모든 데이터 
...
hite = hite.values
print(type(samsung), type(hite))
print(samsung.shape, hite.shape)

np.save('./data/csv/samsung_1120.npy', arr=samsung)
np.save('./data/csv/bit_1120.npy', arr=hite)
