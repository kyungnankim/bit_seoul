...
train_df = pd.read_csv('./mini_project\data/test.csv', header=0, index_col=None,sep=',') #encoding='CP949',
test_df = pd.read_csv('./mini_project\data/train.csv', header=0, index_col=None,sep=',' )
sub  = pd.read_csv('./mini_project/data/submission_sample.csv', header=0, index_col=None,sep=',' )
...
def make_predictions(model, tr_df, tt_df, features_columns, target, params, category_feature=[''], NFOLDS=4, oof_save=False, clip=999, SEED=SEED):
    ...
    oof = np.where(oof>0,gt>0, oof, 0)
    pred = np.where(pred>0,gt>0, pred, 0)
...
test_predictions.to_csv('./mini_project/data/lgb_model.csv', index=False)